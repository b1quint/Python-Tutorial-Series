...
class Pet:
    ...
    def eat(self, food_name):
        ...
        """
        print(' {} eats {}.'.format(self.name, food_name))
...
class Cat(Pet):
    ...
    def eat(self, food_name):
        ...
        if food_name.lower() in ['fish', 'meat']:
            print(' {} just loved the {}!!!'.format(self.name, food_name))
            self.is_hungry = False

        elif food_name.lower() in ['milk']:
...
class Dog(Pet):
    ...
    def eat(self, food_name):
        ...
        if food_name.lower() in ['meat']:
            print(' {} REALLY loves the {}!!!'.format(self.name, food_name))
            self.is_hungry = False

        elif food_name.lower() in ['apple']:
            print(' {} eats {} but it is still hungry...'.format(
                self.name, food_name))
...
if __name__ == '__main__':
    ...
    list_of_pets = [Cat('Charlie'), Cat('Mila'), Dog('Haru')]
    list_of_food = ['Biscuit', 'milk', 'Meat', 'fish', 'Apple']
    ...
    while len(list_of_food) > 0:
        ...
        is_any_pet_hungry = [pet.is_hungry for pet in list_of_pets]

        if not any(is_any_pet_hungry):
            break
